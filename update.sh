...
# Re-install LED server service if the service file was updated
if echo "$CHANGED_FILES" | grep -q "^led-server\.service$"; then
    echo
    echo "LED server service file was updated. Reinstalling systemd service..."
    PATCHED_LED_SERVICE=$(sed \
      -e "s|/home/pi|${HOME}|g" \
      led-server.service)
    if ! echo "${PATCHED_LED_SERVICE}" | sudo tee /etc/systemd/system/led-server.service > /dev/null; then
        echo "WARNING: Failed to reinstall LED server service."
        echo "You can reinstall it manually by running 'bash install.sh'."
    elif ! sudo systemctl daemon-reload; then
        echo "WARNING: Failed to reload systemd daemon."
    elif ! sudo systemctl enable led-server.service; then
        echo "WARNING: Failed to enable LED server service."
    else
        echo "LED server service updated successfully."
        if ! sudo systemctl restart led-server.service; then
            echo "WARNING: Failed to restart LED server. It will be started after the next reboot."
        fi
    fi
# Restart the LED server if only its script changed, so updates take effect
elif echo "$CHANGED_FILES" | grep -q "^src/insectdetect/led\.py$"; then
    if systemctl is-enabled --quiet led-server.service 2>/dev/null; then
        echo
        echo "LED server script was updated. Restarting LED server service..."
        if ! sudo systemctl restart led-server.service; then
            echo "WARNING: Failed to restart LED server service."
        fi
    fi
fi

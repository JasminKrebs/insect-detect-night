...
class FrameSizeConfig(BaseModel):
    """Explicit frame width/height settings for mono capture and web app streaming."""
    width: int = Field(default=544, ge=32, le=3840, multiple_of=32)
    height: int = Field(default=544, ge=2, le=2160, multiple_of=2)
...
class ExposureConfig(BaseModel):
    """Exposure settings for manual mono recording."""
    mode: Literal["auto", "manual"] = "auto"
    time: float = Field(default=3.0, ge=0.1, le=10.0)
    iso: int = Field(default=419, ge=100, le=3200)
...
class CameraConfig(BaseModel):
    ...
    mode: Literal["rgb", "mono"] = "rgb"
    fps: int = Field(default=15, ge=1, le=20)
    image: ImageConfig = ImageConfig()
    resolution_mono: FrameSizeConfig = FrameSizeConfig()
...
class DetectionConfig(BaseModel):
    ...
    model_night: str = "platform_insect-detect-tiled_v2-0-0.tar.xz"
    resolution: FrameSizeConfig = FrameSizeConfig(width=544, height=544)
...
class IntervalConfig(BaseModel):
    ...
    burst_duration: float = Field(default=5.0, ge=1.0, le=10.0)
    timelapse: float = Field(default=600.0, ge=0.0, le=3600.0)
    timelapse_night: float = Field(default=10.0, ge=0.0, le=3600.0)
...
class WebappConfig(BaseModel):
    ...
    resolution: FrameSizeConfig = FrameSizeConfig(width=736, height=720)
    resolution_mono: FrameSizeConfig = FrameSizeConfig(width=544, height=544)
...
class NightConfig(BaseModel):
    """Night-time hardware settings.

    - `ir_intensity`: Manual IR exposure (0.0-1.0) used for camera IR control.
    - `led_brightness`: Brightness level for external LED (0-255).
    """
    ir_intensity: float | None = Field(default=None, ge=0.0, le=1.0)
    led_brightness: int | None = Field(default=None, ge=0, le=255)
...
class AutoRunConfig(BaseModel):
    """Automatically launch a startup script on boot if enabled.
    ...
    """
    enabled: bool = False
    primary: Literal["capture", "capture_auto", "webapp"] = "capture"
    fallback: Literal["capture", "capture_auto", "webapp"] | None = None

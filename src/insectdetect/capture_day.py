...
def main() -> None:
    ...
    logger.info("-------- Capture Day Logger initialized --------")
    ...
    # Force RGB mode for day capture (ensure active values from create_pipeline match RGB)
    try:
        setattr(config.camera, "mode", "rgb")
        logger.info("Forcing camera mode to 'rgb' for day capture")
    except Exception:
        logger.warning("Could not set config.camera.mode to 'rgb'")
    ...
    # Select RGB branch queues for day operation
    q_frames = q_frames_by_mode["rgb"]
    q_tracks = q_tracks_by_mode["rgb"]
    q_camctrl = q_camctrl_by_mode["rgb"]
    frame_size = frame_sizes_by_mode["rgb"]
    sensor_roi = sensor_rois_by_mode["rgb"]

...
from insectdetect.led_client import set_led_detect, set_led_off, set_led_on
from insectdetect.light_sensor import create_get_light_data, init_light_sensor
...
_CONSTRAINT_PATHS: list[str] = [
    ...
    "camera.exposure.time", "camera.exposure.iso",
    ...
    "night.ir_intensity", "night.led_brightness",
...
async def start_camera() -> None:
    ...
    app.state.recording_script = "capture_day"
    ...
    app.state.manual_focus_enabled = app.state.manual_focus_enabled and not app.state.use_mono

    app.state.frame_count = 0
    app.state.fps = 0
    app.state.lens_pos = app.state.config.camera.focus.manual.lens_pos
    app.state.iso_sens = app.state.config.camera.exposure.iso
    app.state.exp_time = app.state.config.camera.exposure.time
    app.state.ir_intensity = app.state.config.night.ir_intensity or 0.0
    app.state.led_on = False
    app.state.led_brightness = app.state.config.night.led_brightness or 50
    app.state.last_led_trigger_time = 0.0
    app.state.lux = 0.0
    ...
    pipeline_result = create_pipeline(app.state.config, stream=True)
    (app.state.pipeline, app.state.q_frames_by_mode, app.state.q_tracks_by_mode, q_syslog,
     app.state.q_camctrl_by_mode, app.state.frame_sizes_by_mode, app.state.nn_input_size,
     app.state.sensor_rois_by_mode, app.state.labels) = pipeline_result

    app.state.q_tracks = app.state.q_tracks_by_mode["mono" if app.state.use_mono else "rgb"]
    app.state.q_syslog = q_syslog

    active_mode = "mono" if app.state.use_mono else "rgb"
    app.state.q_frames = app.state.q_frames_by_mode[active_mode]
    app.state.q_camctrl = app.state.q_camctrl_by_mode[active_mode]
    app.state.frame_size = app.state.frame_sizes_by_mode[active_mode]
    app.state.sensor_roi = app.state.sensor_rois_by_mode[active_mode]

    app.state.pipeline.start()
    app.state.device = app.state.pipeline.getDefaultDevice()
...
async def close_camera() -> None:
    ...
    for queue in ("q_frames", "q_tracks", "q_syslog", "q_camctrl",
                  "q_frames_by_mode", "q_camctrl_by_mode", "frame_sizes_by_mode",
                  "sensor_rois_by_mode"):
    ...
    if getattr(app.state, "led_on", False):
        set_led_off()
        app.state.led_on = False
...
async def restart_camera() -> None:
    """Restart the camera pipeline after a config-driven mode change."""
    await close_camera()
    await asyncio.sleep(0.5)
    await start_camera()

# After camera restart, if mono/manual, re-apply manual settings to camera
async def apply_manual_camera_settings_if_needed() -> None:
    """Re-apply mono IR intensity and manual exposure settings after the pipeline starts."""
    try:
        if app.state.use_mono:
            ir = getattr(app.state.config.night, "ir_intensity", app.state.ir_intensity)
            app.state.ir_intensity = float(ir) if ir is not None else 0.0
        ...
        logger.warning("Failed to re-apply manual camera settings: %s", ex)
...
async def frame_generator() -> AsyncGenerator[bytes, None]:
    ...
    try:
        ...
        last_frame_bytes_length: bytes | None = None
        while getattr(app.state, "q_frames", None):
            ...
            if frame_data is not None:
                ...
                last_frame_bytes_length = frame_bytes_length
                ...
                if elapsed_time > 0.5:
                    ...
                    get_light_data = getattr(app.state, "get_light_data", None)
                    if callable(get_light_data):
            ...
            elif last_frame_bytes is not None and last_frame_bytes_length is not None:
                yield (b"--frame\r\n"
                       b"Content-Type: image/jpeg\r\n"
                       b"Content-Length: " + last_frame_bytes_length + b"\r\n\r\n"
                       + last_frame_bytes + b"\r\n")
            else:
                yield (b"--frame\r\n"
...
async def get_tracker_data() -> list[dict] | None:
    ...
    if tracker_data and not getattr(app.state, "led_on", False):
        now = time.time()
        if now - getattr(app.state, "last_led_trigger_time", 0.0) > 3:
...
async def on_camera_mode_change(e):
    ...
    raw_value = str(getattr(e, "value", "") or "").strip().lower()
    new_mode = "mono" if raw_value in {"mono", "mono left"} else "rgb"

    if app.state.config_updates["camera"].get("mode") == new_mode:
        return

    app.state.config_updates["camera"]["mode"] = new_mode
    app.state.config.camera.mode = new_mode
    app.state.use_mono = (new_mode == "mono")
    app.state.manual_focus_enabled = app.state.manual_focus_enabled and not app.state.use_mono
...
def create_control_elements() -> None:
    ...
    # Select camera mode (RGB or Mono)
    with ui.row(align_items="center").classes("w-full gap-2 mb-2"):
        ui.label("Camera Mode:").classes("font-bold")
        (ui.select(["RGB", "Mono"], on_change=on_camera_mode_change).classes("flex-1")
         .bind_value(app.state.config_updates["camera"], "mode",
                     forward=lambda v: "mono" if v == "Mono" else "rgb",
                     backward=lambda v: "Mono" if v == "mono" else "RGB"))
    ...
    with ui.column().classes("w-full gap-4 mb-2").bind_visibility_from(app.state, "use_mono", value=True):
        ...
        with ui.row(align_items="center").classes("w-full gap-2"):
            ...
            (ui.select(["auto", "manual"], value=app.state.mono_exposure_mode,
                       on_change=on_mono_exposure_mode_change)
             .bind_value(app.state.config_updates["camera"]["exposure"], "mode").classes("min-w-[100px]"))

        # Only show sliders in manual mode
        with ui.column().classes("w-full gap-4").bind_visibility_from(app.state.config_updates["camera"]["exposure"], "mode", value="manual"):#.bind_visibility_from(app.state, "mono_exposure_mode", value="manual"):
            c = FIELD_CONSTRAINTS["night.ir_intensity"]
            assert c["min"] is not None and c["max"] is not None
            with ui.column().classes("w-full"):
                ...
                def on_ir_change(e: ValueChangeEventArguments) -> None: #async def
                    ir_val.set_text(f"{float(e.value):.2f}")
                    app.state.config_updates["night"]["ir_intensity"] = float(e.value)
                    asyncio.create_task(set_ir_intensity(e))#await set_ir_intensity(e)

                app.state.manual_ir_slider = (
                    ui.slider(min=float(c["min"]), max=float(c["max"]), step=0.01,
                              value=app.state.ir_intensity, on_change=on_ir_change)
                    .classes("w-full")
                )

            c = FIELD_CONSTRAINTS["camera.exposure.time"]
            assert c["min"] is not None and c["max"] is not None
            with ui.column().classes("w-full"):
                with ui.row().classes("w-full justify-between items-center"):
                    ui.label("Exposure (ms)").classes("font-bold")
                    exp_val = ui.label(f"{app.state.exp_time:.1f}").classes("text-sm")

                def on_exp_change(e: ValueChangeEventArguments) -> None:
                    exp_val.set_text(f"{float(e.value):.1f}")
                    app.state.config_updates["camera"]["exposure"]["time"] = float(e.value)
                    asyncio.create_task(set_exposure(e))

                app.state.manual_exp_slider = (
                    ui.slider(min=float(c["min"]), max=float(c["max"]), step=0.1,
                              value=app.state.exp_time, on_change=on_exp_change)
                    .classes("w-full")
                )

            c = FIELD_CONSTRAINTS["camera.exposure.iso"]
            assert c["min"] is not None and c["max"] is not None
            with ui.column().classes("w-full"):
                with ui.row().classes("w-full justify-between items-center"):
                ...
                def on_iso_change(e: ValueChangeEventArguments) -> None:
                    iso_val.set_text(str(int(e.value)))
                    app.state.config_updates["camera"]["exposure"]["iso"] = int(e.value)
                    asyncio.create_task(set_iso(e))

                app.state.manual_iso_slider = (
                    ui.slider(min=int(c["min"]), max=int(c["max"]), step=20,
                              value=app.state.iso_sens, on_change=on_iso_change)
                    .classes("w-full")
                )
    ...
    with ui.column().classes("w-full gap-2 mt-4"):
        ...
        def on_led_toggle(e: ValueChangeEventArguments) -> None:
            app.state.led_on = bool(e.value)
            if e.value:
                set_led_on(app.state.led_brightness)
            else:
                set_led_off()

        def on_led_brightness_change(e: ValueChangeEventArguments) -> None:
            brightness_val.set_text(str(int(e.value)))
            app.state.led_brightness = int(e.value)
            app.state.config_updates["night"]["led_brightness"] = int(e.value)
            if app.state.led_on:
                set_led_on(app.state.led_brightness)
        ...
        with ui.column().classes("w-full").bind_visibility_from(app.state, "led_on", value=True):
            ...
            ui.slider(min=0, max=255, step=5, value=app.state.led_brightness,
                      on_change=on_led_brightness_change).classes("w-full")
...
async def on_mono_exposure_mode_change(e: ValueChangeEventArguments) -> None:
    ...
    if mode == "manual":
        ...
        if getattr(app.state, "manual_exp_slider", None):
            app.state.manual_exp_slider.set_value(exp_ms)
        if getattr(app.state, "manual_iso_slider", None):
            app.state.manual_iso_slider.set_value(iso)
        if getattr(app.state, "manual_ir_slider", None):
            app.state.manual_ir_slider.set_value(ir_intensity)

        await set_ir_intensity(ir_intensity)
        await set_exposure(exp_ms)
        await set_iso(iso)

    elif mode == "auto" and getattr(app.state, "q_camctrl", None):
        ctrl = dai.CameraControl()
        ctrl.setAutoExposureEnable()
        app.state.q_camctrl.send(ctrl)


async def set_ir_intensity(e_or_val) -> None:
    ...
    try:
        ...
        app.state.ir_intensity = intensity
        app.state.config_updates["night"]["ir_intensity"] = intensity
        device = getattr(app.state, "device", None)
        if device is None:
            logger.warning("Cannot set IR flood light: OAK device is None")
            return
        device.setIrFloodLightIntensity(intensity)
    except Exception as ex:
        logger.exception("Failed to set IR flood light")


async def set_exposure(e_or_val) -> None:
    """Set manual exposure time in milliseconds."""
    if not getattr(app.state, "q_camctrl", None):
        return
    value = e_or_val.value if hasattr(e_or_val, "value") else e_or_val
    app.state.exp_time = float(value)
    exposure_us = int(app.state.exp_time * 1000)
...
async def set_iso(e_or_val) -> None:
    """Set ISO sensitivity for mono manual exposure."""
    if not getattr(app.state, "q_camctrl", None):
        return
    value = e_or_val.value if hasattr(e_or_val, "value") else e_or_val
    ...
    with ui.grid(columns="auto 1fr").classes("w-full gap-x-5 items-center"):
        ...
        with ui.column().classes("w-full gap-1"):
            ...
            with (ui.column().classes("w-full")
                ...
                (ui.select(["capture", "capture_auto", "webapp"], label="Primary Script").classes("w-full truncate")
                ...
                (ui.select(["capture", "capture_auto", "webapp", "None"], label="Fallback Script").classes("w-full truncate")
...
def sync_config_updates_from_loaded_config(config: AppConfig) -> None:
    """Update the live config-updates dict in place so UI bindings keep working."""
    if not hasattr(app.state, "config_updates") or not isinstance(app.state.config_updates, dict):
        app.state.config_updates = copy.deepcopy(config.model_dump())
        return
    ...
    _merge_dicts(app.state.config_updates, config.model_dump())
    app.state.config = config


async def save_to_file(config_path: Path, suppress_apply_dialog=False) -> AppConfig:
    ...
    saved_config = update_config_yaml(config_path, app.state.config_updates)
    sync_config_updates_from_loaded_config(saved_config)
    ...
    if not suppress_apply_dialog:
        if config_path.name == app.state.config_active:
            ...
            app.state.config = load_config_yaml(config_path)
...
async def save_config(silent=False, suppress_apply_dialog=False) -> AppConfig | None:
    ...
    if action == "new":
        ...
        return app.state.config
    if action == "overwrite":
        return await save_to_file(config_current_path, suppress_apply_dialog=suppress_apply_dialog)
    return None
...
async def start_recording() -> None:
    ...
    with ui.dialog() as dialog, ui.card():
        ...
        recording_script = ui.select(
            {"capture_day": "Day capture", "capture_night": "Night capture"},
    ...
    if start_rec:
        app.state.recording_script = recording_script.value
...
async def on_app_shutdown() -> None:
    ...
    if getattr(app.state, "start_recording", False):
        recording_script = getattr(app.state, "recording_script", "capture_day")
        subprocess_log(f"{recording_script}.py")

        with open(LOGS_PATH / "subprocess.log", "a", encoding="utf-8") as log_file_handle:
            subprocess.Popen(
                [str(UV), "run", recording_script],

...
from insectdetect.led_client import set_led_burst, set_led_off, set_led_on
from insectdetect.light_sensor import init_light_sensor, create_get_light_data
...
class RecordingContext:
    ...
    q_frames_mono: dai.MessageQueue
    q_tracks: dai.MessageQueue
    q_tracks_mono: dai.MessageQueue
    q_syslog: dai.MessageQueue
    q_camctrl: dai.InputQueue
    q_camctrl_mono: dai.InputQueue
...
def _run_recording(
    ...
    with (
        ...
        metadata_writer = csv.DictWriter(metadata_file, fieldnames=[
            ...
            "lens_position", "iso_sensitivity", "exposure_time",
            "ir_intensity", "led_brightness", "lux", "infrared", "visible", "filename"
        ])
        metadata_writer.writeheader()

        # Upload pipeline to OAK device and start camera
        ctx.pipeline.start()
        device = ctx.pipeline.getDefaultDevice()

        # Apply configured IR floodlight intensity
        ...
        if ir_intensity is not None:
            try:
                ...
                logger.exception("Failed to set IR floodlight intensity to %s", ir_intensity)
        ...
        tl_interval = config.recording.interval.timelapse_night
        ...
        last_detection_capture = start_time - det_interval
        ...
        track_active_since: dict[int, float] = {}
        track_cooldowns: dict[int, float] = {}

        try:
            led_on = False
            led_burst_end = 0.0
            detection_burst_active = False
            detection_burst_end = 0.0
            ...
            while (
                ...
                for track_id, cooldown_until in list(track_cooldowns.items()):
                    if current_time >= cooldown_until:
                        ...
                        track_active_since.pop(track_id, None)
                triggered_capture = current_time >= next_capture
                timelapse_capture = current_time >= last_capture + tl_interval


                # For night: detection runs on mono tracker; use RGB frames for detection saves and mono frames for timelapse
                frame_rgb = cast(dai.ImgFrame | None, ctx.q_frames.tryGet())
                frame_mono = cast(dai.ImgFrame | None, ctx.q_frames_mono.tryGet())
                track = cast(dai.Tracklets | None, ctx.q_tracks.tryGet())
                frame = None
                if triggered_capture:
                    frame = frame_rgb
                elif timelapse_capture:
                    frame = frame_mono
                ...
                if frame is not None and (triggered_capture or timelapse_capture):
                    ...
                    IR_INTENSITY = getattr(config, 'night', None) and getattr(config.night, 'ir_intensity', None)
                    LED_BRIGHTNESS = getattr(config, 'night', None) and getattr(config.night, 'led_brightness', None)
                    ...
                    if track is not None:
                        ...
                        for tracklet in track.tracklets:
                            ...
                            if tracklet_status in {"TRACKED", "NEW"}:
                                ...
                                if tracklet_id in track_cooldowns:
                                    continue
                                ...
                                if tracklet_id not in track_active_since:
                                    track_active_since[tracklet_id] = current_time
                                elif current_time - track_active_since[tracklet_id] >= 10.0: # TO-DO: adjust cool down mechanism
                                    track_cooldowns[tracklet_id] = current_time + 10.0
                                    track_active_since.pop(tracklet_id, None)
                                    logger.info(
                                        "Suppressing track %s for 10s after persistent detection >10s",
                                        tracklet_id
                                    )
                                    continue
                    ...
                    # Detection burst logic: start burst when track_active
                    if track_active or timelapse_capture:
                        if track_active:
                            if detection_start_time is None:
                                detection_start_time = current_time
                                detection_burst_active = True
                                detection_burst_end = current_time + getattr(config.recording.interval, 'burst_duration', 5)
                                next_capture = current_time if current_time >= last_detection_capture + det_interval else last_detection_capture + det_interval
                                if not led_on:
                                    try:
                                        set_led_on(getattr(config.night, 'led_brightness', None))
                                    except Exception:
                                        pass
                                    led_on = True
                                    led_burst_end = current_time + getattr(config.recording, 'burst_duration', 5)
                            elif not detection_burst_active:
                                detection_burst_active = True
                                detection_burst_end = current_time + getattr(config.recording, 'burst_duration', 5)
                                next_capture = current_time if current_time >= last_detection_capture + det_interval else last_detection_capture + det_interval
                                if not led_on:
                                    try:
                                        set_led_on(getattr(config.night, 'led_brightness', None))
                        ...
                        # Save an RGB detection frame (during burst) or a mono timelapse frame
                        frame = None
                        frame_source = None
                        if detection_burst_active and current_time < detection_burst_end:
                            if frame_rgb is not None and current_time >= next_capture:
                                frame = frame_rgb
                                frame_source = 'rgb'
                                last_detection_capture = current_time
                                next_capture = current_time + det_interval

                        elif timelapse_capture: # TO-DO: change it to if (but then frame logic needs to be adjusted)
                            if frame_mono is not None:
                                frame = frame_mono
                                frame_source = 'mono'
                                last_capture = current_time
                                # optionally save model input frame here if available

                        if frame is not None:
                            # Save frame (RGB detection or mono timelapse)
                            save_future = executor.submit(save_encoded_frame, frame, ctx.session_path, ctx.timelapse_path, file_stem, 'detection' if frame_source == 'rgb' else 'timelapse')
                            #if frame_source == 'rgb':
...
def main() -> None:
    ...
    logger.info("-------- Capture Night Logger initialized --------")
    ...
    # Force mono mode for night capture (ensure active values from create_pipeline match mono)
    try:
        setattr(config.camera, "mode", "mono")
        logger.info("Forcing camera mode to 'mono' for night capture")
    except Exception:
        logger.warning("Could not set config.camera.mode to 'mono'")

    # Use the dedicated night model if it is configured in the detection section.
    try:
        if getattr(config.detection, 'model_night', None):
            config.detection.model = config.detection.model_night
            logger.info("Using night detection model: %s", config.detection.model_night)
    except Exception:
        logger.debug("No detection.model_night found in config; using default detection.model")
    ...
    (pipeline, q_frames_by_mode, q_tracks_by_mode, q_syslog, q_camctrl_by_mode,
     frame_sizes_by_mode, nn_input_size, sensor_rois_by_mode, labels) = create_pipeline(config, stream=False)

    # For night: detection on the mono tracker, while high-quality RGB frames are saved on triggers.
    q_frames_rgb = q_frames_by_mode["rgb"]
    q_frames_mono = q_frames_by_mode["mono"]
    q_tracks_mono = q_tracks_by_mode["mono"]
    q_camctrl_mono = q_camctrl_by_mode["mono"]
    frame_size_mono = frame_sizes_by_mode["mono"]
    sensor_roi_mono = sensor_rois_by_mode["mono"]

    # Use RGB frames for saving, but use mono branch tracking/control for detection logic.
    ctx = RecordingContext(
        ...
        pwr=pwr,
        pipeline=pipeline,
        q_frames=q_frames_rgb,
        q_frames_mono=q_frames_mono,
        q_tracks=q_tracks_mono,
        q_tracks_mono=q_tracks_mono,
        q_syslog=q_syslog,
        q_camctrl=q_camctrl_mono,
        q_camctrl_mono=q_camctrl_mono,
        frame_size=frame_size_mono,
        nn_input_size=nn_input_size,
        sensor_roi=sensor_roi_mono,
        labels=labels
    )

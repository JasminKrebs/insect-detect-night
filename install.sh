...
# Enable I2C interface for the TSL2591 light sensor
if command -v raspi-config >/dev/null 2>&1; then
    echo "Enabling I2C interface for the light sensor..."
    if ! sudo raspi-config nonint do_i2c 0; then
        echo "ERROR: Failed to enable I2C. Enable it manually by running 'sudo raspi-config'."
        exit 1
    fi
else
    echo "WARNING: raspi-config not found. Make sure I2C is enabled for the light sensor."
fi
echo
...
# Install and enable systemd service for the LED server (controls LEDs via UNIX socket)
echo
echo "[+] Installing systemd service for LED server..."

# Patch service file with actual home directory
PATCHED_LED_SERVICE=$(sed \
  -e "s|/home/pi|${HOME}|g" \
  led-server.service)

if ! echo "${PATCHED_LED_SERVICE}" | sudo tee /etc/systemd/system/led-server.service > /dev/null; then
    echo "ERROR: Failed to install LED server service file."
    exit 1
fi
if ! sudo systemctl daemon-reload; then
    echo "ERROR: Failed to reload systemd daemon."
...
if ! sudo systemctl enable led-server.service; then
    echo "ERROR: Failed to enable LED server service."
    exit 1
fi
echo "LED server service installed and enabled."
if ! sudo systemctl start led-server.service; then
    echo "ERROR: Failed to start LED server. It will be started automatically after the next reboot."
fi
